>bash本身没有提供正则表达式，shell语法中使用的是通配符，通常是一些命令和软件包（sed/awk）使用的才是正则表达式。
...
一个正式表达式是一个字符串.字符串里的字符被称为元字符，它们可以表示了比它们字面上看起来的意思更丰富的含义。正则表达式是由字符和元字符组合成的字符集，它们匹配(或指定)一个模式。
Shell 中可以使用正则表达式如下：
